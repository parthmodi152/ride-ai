...
export class MockUberAdapter implements PlatformAdapter {
  ...
  async getEstimates(params: {
    ...
    const distance = Math.sqrt(
      Math.pow(params.pickup.lat - params.dropoff.lat, 2) +
        Math.pow(params.pickup.lng - params.dropoff.lng, 2),
    )
  ...
  async cancelRide(params: { tripId: string }): Promise<CancelResult> {
    ...
    const fee = trip.status === 'IN_PROGRESS' ? 5.0 : trip.status === 'ARRIVING' ? 2.5 : undefined
